...
Elasticity measures the relative change in the dependent variable due to a relative change in the independent variable. Here, we calculate the **elasticity of house price (PRICE) with respect to house size (SQFT)** using the formula: $ E = \frac{dPRICE}{dSQFT} \times \frac{SQFT}{PRICE} $
