## C03Q01 a ~ e (Problems)
![C03Q01](https://github.com/user-attachments/assets/896f4ae6-674b-4d9d-a352-9d41415d4766)

## C03Q07 a ~ g (Problems)
![C03Q07](https://github.com/user-attachments/assets/731a1b2b-441c-4c7e-aeeb-f2420f056dd3)

## C03Q17 a ~ d (Problems)
![C03Q17](https://github.com/user-attachments/assets/c8b47113-c1eb-443c-a3e5-7965a106d4dd)

## C03Q19 a ~ e (Computer Exercise)
![C03Q19](https://github.com/user-attachments/assets/a17051f0-deba-45f8-8753-9d6d255ae49b)

---
C03Q01 a ~ e (Problems)

1992 年有 64 個國家參加奧運並至少獲得一面獎牌。設 **MEDALS** 為該國家獲得的獎牌總數，**GDPB** 為該國家的 GDP（以 1995 年美元的十億單位計算）。用於解釋獎牌數量的線性回歸模型為：

$$
\ \text{MEDALS} = \beta_1 + \beta_2 \text{GDPB} + e \
$$

估計的關係式為：

$$
\ \text{MEDALS} = b_1 + b_2 \text{GDPB} \ = 7.61733 + 0.01309 \text{GDPB} \
$$

                        (se)                    (2.38994) (0.00215)   (XR3.1)


**a.** 我們希望檢驗獎牌數與 GDP 之間是否沒有關係，對立假設為兩者之間存在正向關係。請根據模型參數陳述虛無假設與對立假設。

**b.** 針對 (a) 的檢定，計算其檢定統計量，並說明當虛無假設為真時其分布為何？

**c.** 若對立假設為真，檢定統計量的分布會如何變化？相較於標準的 t-分布，該分布會向左還是向右偏移？【提示：當虛無假設為真時，b2 的期望值是多少？當對立假設為真時，又是多少？】

**d.** 在 1% 顯著性水準下，在哪些 t-統計量的值範圍內我們會拒絕 (a) 的虛無假設？在哪些值範圍內我們不會拒絕虛無假設？

**e.** 以 1% 顯著性水準進行 (a) 的 t-檢定，並得出經濟結論。1% 顯著性水準在這個例子中代表什麼意義？

---

# 假說檢定：GDP 與獎牌數量的關係

## (a) 陳述假說
- **虛無假說 (H₀):** 模型參數中，認為GDP對獎牌數量沒有影響，也就是說 β₂ = 0。
- **對立假說 (H₁):** GDP對獎牌數量有正向影響，也就是說 β₂ > 0。

## (b) 檢定統計量及其分布
- **檢定統計量計算公式：**

  t = (b₂ - 0) / se(b₂)
  
- **代入數值：**

  t = 0.01309 / 0.00215 ≈ 6.086
  
- **分布：**  
  在虛無假說 β₂ = 0 下，該統計量服從自由度 n - 2 的 t 分布，此處自由度為 64 - 2 = 62。

## (c) 對立假說下的檢定統計量分布變化
當對立假說 β₂ > 0 成立時，b₂ 的期望值大於 0，因此檢定統計量 t 的分布會成為一個非中心 t 分布，其非中心參數為正值。相對於虛中心的 t 分布（中心 t 分布），這個分布會向右移動，也就是說大部分的概率質量將落在較大的 t 值那邊。

## (d) 1% 顯著水準下拒絕與不拒絕虛無假說的 t 統計量值
- 由於這是一個單尾檢定（右尾檢定），在 1% 的顯著水準下，我們需要查找 t₀.₉₉ 的臨界值。
- 對於自由度為 62 的 t 分布，臨界值大約為 2.39（參考 t 分布表）。
  - **拒絕域：** 當 t > 2.39 時，拒絕虛無假說。
  - **不拒絕域：** 當 t ≤ 2.39 時，不拒絕虛無假說。

## (e) 執行 t 檢定並說明經濟意涵
- **計算結果：**  
  我們計算得到 t ≈ 6.086，顯著大於臨界值 2.39。
  
- **檢定結論：**  
  在 1% 的顯著水準下，我們拒絕 H₀。
  
- **經濟意涵：**  
  結果顯示GDP與獎牌數量之間存在正向且顯著的關係。也就是說，擁有較高GDP的國家更可能贏得較多的獎牌。

- **1% 顯著水準的意義：**  
  此顯著水準表示在拒絕真實虛無假說（犯下第一類錯誤）的機率僅有1%，意味著僅在極不可能的情況下才會錯誤地拒絕 H₀。

---

C03Q07 a ~ g (Problems)

我們有 2008 年的數據，其中 **INCOME** 代表人均收入（以千美元計），**BACHELOR** 代表擁有學士學位或以上人口的比例，涵蓋美國 50 個州和哥倫比亞特區，共 N = 51 筆觀察數據。對 INCOME 進行 BACHELOR 的簡單線性回歸模型結果如下：

INCOME = (a) + 1.029 BACHELOR

    se =   (2.672)  (c)

    t =    (4.31)  (10.75)


**a.** 根據提供的信息，計算估計的截距。請詳細列出計算過程。

**b.** 描述回歸關係的變化趨勢。這是遞增還是遞減的關係？這是正向還是反向關係？這種變化是以固定速率進行，還是呈現加速或減速變化？

**c.** 使用提供的信息，計算斜率係數的標準誤差，並展示計算過程。

**d.** 計算當截距參數等於 10 時的 t-統計量值是多少？

**e.** 在 (d) 部分中，針對檢定截距參數等於 10 的雙尾檢定，p-值為 0.572。請在圖示中標示 p-值，並顯示當 α = 0.05 時的拒絕區間。

**f.** 建構 99% 信賴區間以估計斜率，並解釋該區間的意義。

**g.** 以 5% 顯著性水準檢定斜率係數是否等於 1，對立假設為斜率係數不等於 1。根據本問題的背景，請陳述此檢定的經濟意涵。

---

## (a) 計算估計的截距
迴歸方程式為：  
  **INCOME = (a) + 1.029 × BACHELOR**

題目提供截距的 t 值為 4.31，其標準誤為 2.672。  
根據 t 統計量公式：  
  t = (估計值 − 0) / 標準誤  
因此，估計截距 = t × 標準誤  
  計算： a = 4.31 × 2.672 ≈ 11.53  

**答案：(a) 估計截距約為 11.53**


## (b) 畫出估計的迴歸關係圖及解釋
估計迴歸式：  
  **INCOME = 11.53 + 1.029 × BACHELOR**

**關係解釋：**  
- **上升還是下降？**  
  由於斜率為正 (1.029 > 0)，隨著 BACHELOR 上升，INCOME 上升。
- **正向或反向關係？**  
  斜率為正，因此是正向關係。
- **變化速率？**  
  這是一條線性關係，表示每增加1個百分點，收入增加約 1.029 千美元，即以固定速率變化。

**圖示概念：**  
畫一條直線，橫軸為 BACHELOR，縱軸為 INCOME，截距約 11.53，斜率 1.029。


## (c) 計算斜率係數的標準誤
斜率係數給定為 1.029，且其 t 值為 10.75。  
根據 t 統計量公式：  
  t = (估計斜率 − 0) / 標準誤  
因此，標準誤 = 斜率 / t  
  計算： se = 1.029 / 10.75 ≈ 0.0958  

**答案：(c) 斜率標準誤約為 0.0958**


## (d) 檢定截距參數等於 10 的 t 統計量
**虛無假說：** 截距 = 10  
估計截距約為 11.53，標準誤為 2.672。  

t 統計量計算：  
  t = (11.53 − 10) / 2.672 ≈ 1.53 / 2.672 ≈ 0.572  

**答案：(d) t 統計量約為 0.572**


## (e) 畫出 p 值及拒絕域（α = 0.05）
已知雙尾檢定時針對截距等於 10 的 p 值為 0.572。

**圖示說明：**  
- 畫出一個 t 分布曲線，其中心在 0。  
- 在 t = ±臨界值處劃分拒絕域（對於 α = 0.05，雙尾檢定下，每尾的顯著水準為 0.025；例如對 df = 49，臨界值大約是 ±2.01）。  
- 標示 t = 0.572（及 t = −0.572）的區域，該區域所對應的累積面積與尾部面積總和即為 p 值 0.572。

**結論：**  
因 t 值 (0.572) 落在接受域內，故不拒絕虛無假說。

**答案：(e)** 請參照圖示：在 t 分布上，拒絕域位於 t < -2.01 和 t > 2.01，而 t = 0.572 落在中間接受區間內，該兩側總面積約為 0.572。


## (f) 建構 99% 信賴區間（斜率）並解釋
估計斜率 = 1.029，標準誤 ≈ 0.0958。  
自由度 df = 51 − 2 = 49。  
查找 df = 49 下 99% 信賴區間的臨界 t 值（雙尾）：大約 t* ≈ 2.68。

信賴區間計算：  
  1.029 ± 2.68 × 0.0958  
  邊際誤差 = 2.68 × 0.0958 ≈ 0.257  
  因此，區間約為 (1.029 − 0.257, 1.029 + 0.257) = (0.772, 1.286)

**解釋：**  
我們有 99% 的信心相信，每增加1個百分點的 BACHELOR，INCOME（以千美元計）將增加介於約 0.772 至 1.286 千美元之間。

**答案：(f) 斜率 99% 信賴區間為 (0.772, 1.286)**


## (g) 斜率檢定：檢定斜率是否等於 1
**虛無假說 H₀：** 斜率 = 1  
**對立假說 H₁：** 斜率 ≠ 1  

估計斜率 = 1.029，標準誤 ≈ 0.0958。  
t 統計量計算：  
  t = (1.029 − 1) / 0.0958 ≈ 0.029 / 0.0958 ≈ 0.303  

對於 df = 49，5% 顯著水準下（雙尾）臨界值約為 ±2.01。  
因為 |0.303| < 2.01，所以不拒絕虛無假說。

**經濟意涵：**  
沒有足夠的證據表明 BACHELOR 對 INCOME 的影響與 1（即每增加1個百分點，收入增加 1 千美元）的假設有顯著差異。換言之，數據支持一個近似於一比一的正向關係。

**答案：(g) 在 5% 顯著水準下，檢定結果顯示不拒絕斜率為 1 的虛無假說，意即統計上無顯著差異，支持每增加 1 個百分點的 BACHELOR 與 INCOME 增加約 1 千美元的關係。  

---
C03Q17 a ~ d (Problems)

考慮回歸模型  

$$
\ \text{WAGE} = \beta_1 + \beta_2 \text{EDUC} + e \  
$$

其中 **WAGE** 為 2013 年美國每小時工資（以美元計算），**EDUC** 代表受教育年數。該模型分別針對城市地區與鄉村地區的個體進行估計。

**城市:**  
 
$$
\ \text{WAGE} = -10.76 + 2.46 \text{EDUC}, \quad N = 986 \ 
$$

                          (se) =     (2.27) (0.16)

**鄉村:**  

$$
\ \text{WAGE} = -4.88 + 1.80 \text{EDUC}, \quad N = 214 \ 
$$

                          (se) =     (3.29) (0.24)

**a.** 根據城市回歸結果，檢定回歸斜率是否等於 1.80，對立假設為斜率大於 1.80，顯著性水準 α = 0.05。請列出所有步驟，包括臨界區域的圖示，並陳述結論。

**b.** 根據鄉村回歸結果，計算當 **EDUC = 16** 時的 95% 置信區間估計值。已知標準誤差為 0.833，請展示計算過程，並考慮截距與斜率參數的協方差為 -0.761。

**c.** 根據城市回歸結果，計算當 **EDUC = 16** 時的 95% 置信區間估計值。已知截距與斜率參數的協方差為 -0.345。請比較城市回歸與鄉村回歸的區間估計範圍，城市回歸的區間是否比鄉村回歸的區間更寬或更窄？這樣的結果是否合理？請解釋。

**d.** 根據鄉村回歸結果，檢定截距參數 β₁ 是否等於 4 或更大，對立假設為截距小於 4，在 1% 顯著性水準下。

---

## (a) 城市區迴歸：檢驗斜率是否大於 1.80

**給定資訊：**  
- 估計斜率：𝛽̂₂ = 2.46  
- 斜率標準誤：se(𝛽̂₂) = 0.16  
- 樣本數：N = 986（自由度 df = 986 − 2 = 984）

**檢定假說：**  
- **虛無假說 H₀：** 𝛽₂ = 1.80  
- **對立假說 H₁：** 𝛽₂ > 1.80 （單尾右側檢定）

**計算 t 統計量：**  
  t = (2.46 − 1.80) / 0.16 = 0.66 / 0.16 ≈ 4.125

**臨界值：**  
- 對於 α = 0.05 的單尾檢定，由於自由度很大，t 分布近似標準常態，此時臨界值約為 1.645  
  （或查 t 表得 t₀.₉₅,984 ≈ 1.645）

**決策規則：**  
- 若 t > 1.645 則拒絕虛無假說

**結果：**  
- 4.125 > 1.645，因此拒絕 H₀

**圖示概念：**  
- 畫一個 t 分布曲線（中心在 0）  
- 在右尾劃出臨界值 1.645 之後的區域作為拒絕區  
- 標示出觀察到的 t 值 4.125 落在拒絕區內

**結論：**  
在 5% 顯著水準下，有足夠證據支持城市區中教育年限對工資的影響大於 1.80，即斜率顯著大於 1.80。


## (b) 鄉村區：對 EDUC = 16 預測 WAGE 的 95% 信賴區間

**給定資訊：**  
- 鄉村迴歸：  
  𝛽̂₁ = −4.88, 𝛽̂₂ = 1.80  
- 標準誤：  
  se(𝛽̂₁) = 3.29, se(𝛽̂₂) = 0.24  
- 協方差：  
  Cov(𝛽̂₁, 𝛽̂₂) = −0.761  
- EDUC = 16  
- 題中給定的預測標準誤最終採用 SE = 0.833

**預測值計算：**  
  WAGÊ = −4.88 + 1.80 × 16 = −4.88 + 28.8 = 23.92

**信賴區間構造：**  
- 自由度：df = 214 − 2 = 212  
- 95% 信賴區間下的 t 臨界值約為 t₀.₉₇₅,212 ≈ 1.97  
- 邊際誤差：ME = 1.97 × 0.833 ≈ 1.64  
- 因此，95% 信賴區間為：  
  (23.92 − 1.64, 23.92 + 1.64) = (22.28, 25.56)


## (c) 城市區：對 EDUC = 16 預測 WAGE 的 95% 信賴區間及比較

**給定資訊：**  
- 城市迴歸：  
  𝛽̂₁ = −10.76, 𝛽̂₂ = 2.46  
- 標準誤：  
  se(𝛽̂₁) = 2.27, se(𝛽̂₂) = 0.16  
- 協方差：  
  Cov(𝛽̂₁, 𝛽̂₂) = −0.345  
- EDUC = 16  
- 樣本數：N = 986（自由度 df = 984，極大，可採用標準常態近似）

**預測值計算：**  
  WAGÊ = −10.76 + 2.46 × 16 = −10.76 + 39.36 = 28.60

**預測標準誤計算：**  
採用公式  
  SE(WAGÊ) = √[ se(𝛽̂₁)² + (16)² × se(𝛽̂₂)² + 2 × 16 × Cov(𝛽̂₁, 𝛽̂₂) ]  
計算步驟：  
  (2.27)² ≈ 5.1529  
  (0.16)² = 0.0256 → 16² = 256 → 256 × 0.0256 = 6.5536  
  2 × 16 × (−0.345) = 32 × (−0.345) = −11.04  
  合計：5.1529 + 6.5536 − 11.04 ≈ 0.6665  
  開根號：√0.6665 ≈ 0.817

**信賴區間構造：**  
- 臨界值：由於自由度很大，約為 1.96  
- 邊際誤差：ME = 1.96 × 0.817 ≈ 1.60  
- 95% 信賴區間為：  
  (28.60 − 1.60, 28.60 + 1.60) = (27.00, 30.20)

**比較與解釋：**  
- 鄉村區信賴區間為 (22.28, 25.56)  
- 城市區信賴區間為 (27.00, 30.20)  
  從預測標準誤來看，城市區約 0.817 與鄉村區給定的 0.833 相近；  
  但城市區的樣本量較大（986 vs. 214），通常會產生更精確的估計，進而使信賴區間較窄。  
  此外，兩迴歸的點估計不同，城市區預測工資較高，這與直覺相符。


## (d) 鄉村區：檢驗截距是否小於 4

**給定資訊（鄉村區）：**  
- 截距估計：𝛽̂₁ = −4.88  
- 截距標準誤：se(𝛽̂₁) = 3.29  
- 樣本數：N = 214（自由度 df = 214 − 2 = 212）

**檢定假說：**  
- **虛無假說 H₀：** 𝛽₁ ≥ 4  
- **對立假說 H₁：** 𝛽₁ < 4 （單尾左側檢定）

**計算 t 統計量：**  
  t = (𝛽̂₁ − 4) / se(𝛽̂₁) = (−4.88 − 4) / 3.29 = (−8.88) / 3.29 ≈ −2.70

**臨界值：**  
- 在 1% 顯著水準下（單尾左側檢定），對於 df = 212，臨界 t 值約為 −2.33

**決策規則：**  
- 若 t < −2.33 則拒絕虛無假說

**結果與結論：**  
- 由於 −2.70 < −2.33，故拒絕 H₀  
- 在 1% 顯著水準下，有足夠證據支持鄉村區的截距參數小於 4。

---

C03Q19 a ~ e (Computer Exercise)

某汽車旅館的業主發現施工時使用了有缺陷的產品，修復缺陷共花費 7 個月，在此期間，這間擁有 100 個房間的旅館中，每月約有 14 個房間無法使用。相關數據存於 `motel` 檔案。

**a.** 繪製 `MOTEL_PCT` 和 `COMP_PCT` 隨時間 `TIME` 的變化圖。你對於這些佔用率隨時間的變化有何看法？它們是否呈現共同變動的趨勢？哪一個的佔用率似乎較高？估計回歸模型 MOTEL_PCT = β₁ + β₂ COMP_PCT + e 
並構造 β₂ 參數的 95% 信賴區間。我們對於 `MOTEL_PCT` 與 `COMP_PCT` 之間的關聯估計得精確嗎？請解釋你的推理。

**b.** 建立 90% 信賴區間以估計當 `COMP_PCT = 70` 時，旅館的預期佔用率 `MOTEL_PCT`。

**c.** 在線性回歸模型 MOTEL_PCT = β₁ + β₂ COMP_PCT + e 中,檢定虛無假設 H0: β₂ ≦ 0，對立假設為 Ha: β₂ > 0，顯著性水準 α = 0.01。討論你的結論，並清楚定義所使用的檢定統計量及拒絕域。

**d.** 在線性回歸模型 MOTEL_PCT = β₁ + β₂ COMP_PCT + e 中，檢定虛無假設 H0: β₂ = 1，對立假設為 Ha: β₂ ≠ 1，顯著性水準 α = 0.01。如果虛無假設為真，這對於旅館的佔用率與其競爭對手的佔用率有何含義？討論你的結論，並清楚定義所使用的檢定統計量及拒絕域。

**e.** 計算 `MOTEL_PCT` 對 `COMP_PCT` 進行回歸的最小平方法殘差，並將其對 `TIME` 作圖。圖中是否有異常特徵？在時間區間 17–23（2004 年 7 月至 2005 年 1 月），殘差的主要符號是什麼？

---

# 逐步解答說明

以下解答說明各部分應如何處理（注意：由於未提供實際數據，以下解答多採用一般性說明與公式表示，具體數值需依據實際估計結果而定）。

## (a) 比較與迴歸分析及 β₂ 信賴區間

1. **資料繪圖與趨勢比較**  
   - 將 MOTEL_PCT 與 COMP_PCT 分別對 TIME 畫圖，並將兩者疊加在同一圖中。  
   - 觀察兩條曲線隨時間變化的趨勢。如果兩者大致呈現相同的上升或下降趨勢，表示它們 tend to move together；反之，若走勢差異較大，則可能獨立波動。  
   - 同時注意哪一系列的入住率較高，例如若 COMP_PCT 曲線持續高於 MOTEL_PCT，則可認為競爭對手的入住率較高。

2. **迴歸估計**  
   - 估計下列迴歸模型：  
     ```
     MOTEL_PCT = β₁ + β₂ COMP_PCT + e
     ```  
   - 使用最小平方法 (OLS) 得到 β̂₁ 與 β̂₂ 的點估計，並取得 β̂₂ 的標準誤 (SE(β̂₂))。

3. **構造 β₂ 的 95% 信賴區間**  
   - 信賴區間公式：  
     ```
     β̂₂ ∈ [ β̂₂ − t(0.975, df) × SE(β̂₂), β̂₂ + t(0.975, df) × SE(β̂₂) ]
     ```  
   - 若信賴區間較窄，表示 β₂ 的估計較為精確；若較寬則代表估計的不確定性較高。  
   - 若信賴區間不包含 0，則說明 COMP_PCT 與 MOTEL_PCT 呈現顯著正相關。

## (b) 預測值的 90% 信賴區間（當 COMP_PCT = 70）

1. **預測點計算**  
   - 利用模型預測：  
     ```
     MOTEL_PCT^ = β̂₁ + β̂₂ × 70
     ```  
   - 其中 COMP_PCT 設為 70，即可算出 MOTEL_PCT 的預測值。

2. **預測標準誤與信賴區間構造**  
   - 90% 信賴區間公式：  
     ```
     MOTEL_PCT^ ± t(0.95, df) × SE(MOTEL_PCT^)
     ```  
   - 其中 SE(MOTEL_PCT^) 是根據迴歸預測公式計算的預測標準誤（包含點估計變異與樣本變異）。  
   - 此區間表示我們有 90% 的信心，當 COMP_PCT = 70 時，真正的平均 MOTEL_PCT 落在該區間內。

## (c) 檢驗 β₂ > 0（線性關係是否正向）

1. **檢定假說**  
   - 虛無假說 H₀：β₂ ≦ 0  
   - 對立假說 Hₐ：β₂ > 0

2. **計算檢定統計量**  
   - t 統計量公式：  
     ```
     t = (β̂₂ − 0) / SE(β̂₂)
     ```

3. **拒絕域與結論**  
   - 在 α = 0.01 的單尾檢定中，若 t > t(0.99, df) 則拒絕 H₀。  
   - 若計算出的 t 統計量超過臨界值，即可認為 COMP_PCT 與 MOTEL_PCT 之間呈現顯著正向關聯。

## (d) 檢驗 β₂ 是否等於 1

1. **檢定假說**  
   - 虛無假說 H₀：β₂ = 1  
   - 對立假說 Hₐ：β₂ ≠ 1

2. **計算檢定統計量**  
   - t 統計量公式：  
     ```
     t = (β̂₂ − 1) / SE(β̂₂)
     ```

3. **拒絕域與解釋**  
   - 在 α = 0.01 的雙尾檢定中，若 |t| > t(0.995, df) 則拒絕 H₀。  
   - 若 H₀ 為真，意味著競爭對手入住率每增加 1 個百分點，汽車旅館入住率也會增加 1 個百分點，即兩者呈一比一關係。  
   - 根據 t 統計量與臨界值比較，判斷是否拒絕 H₀，從而了解實際估計的 β₂ 是否顯著偏離 1。

## (e) 殘差分析

1. **計算殘差**  
   - 每一觀察值的殘差公式：  
     ```
     eᵢ = MOTEL_PCTᵢ − (β̂₁ + β̂₂ × COMP_PCTᵢ)
     ```

2. **殘差對時間繪圖**  
   - 將所有觀察的殘差依據 TIME 畫圖，檢查是否存在系統性模式（例如趨勢、週期性變化或異常點）。

3. **特別注意的時間段**  
   - 特別檢查 2004 年 7 月至 2005 年 1 月（TIME 代碼 17–23）之間的殘差情形。  
   - 若該段期間大部分殘差為正，表示實際 MOTEL_PCT 高於模型預測；若大部分為負，則表示實際 MOTEL_PCT 低於預測。  
   - 此結果可能揭示在該段期間因外部因素（例如修繕工程等）影響了汽車旅館入住率。

---

