...
#5.33(c)
b2<-coef(model1)["educ"]
b3<-coef(model1)["I(educ^2)"]
b6<-coef(model1)["I(educ * exper)"]
...
#5.33(e)
b4<-coef(model1)["exper"]
b5<-coef(model1)["I(exper^2)"]
b6<-coef(model1)["I(educ * exper)"]
...
#5.33(g)
exper_d_new <- cps5_small$exper + 8  
exper_s_new <- cps5_small$exper + 8
...
t_crit <- qt(0.05, df = model1$df.residual)
