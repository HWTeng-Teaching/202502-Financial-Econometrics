...
係數在兩年 非常接近；使用單一年份迴歸，等於 假設每位受訪者的截距與斜率都相同，僅允許截距隨年份移動，忽視個體間的不觀察異質性（即假設 𝑢𝑖=0)
...
𝛽^2=0.0575, 𝛽^3=−0.0012,𝛽^4=−0.3261,𝛽^5=0.0822 與 OLS（平均值）相比，SOUTH 的效果差最多（從 ≈ −0.23 降到 −0.33）。


**(d)檢定「無個體效果」的 𝐹-統計量 (式 15.20)**

𝐹𝐻0=11.68(𝑛−1,𝑛𝑇−𝑛−k)=(715,1432-6)；1 % 臨界值 𝐹0.01,715,1426≈1.34

11.68≫1.34 ⇒ 拒絕無個體效果：必須用 FE 或 RE，不可用 pooled OLS。

**(e)　叢集 (個體) — 強健標準誤**

Column (4) 只是將 FE 的殘差對 個體 叢集，放寬了「同一人不具相關性」假設。

Robust SE 大多 略大（EXPER, EXPER2差異最小；SOUTH 差距最大）。


**(f)　RE vs. FE 的 Hausman 檢定**

FE-RE 係數只在 UNION 不同（其他幾乎一致）。

Hausman 𝜒2(5)=0.09 < 3.841 ⇒ 未拒絕 𝐻0（RE 一致）。→ 可以採用 隨機效果 (b)。
