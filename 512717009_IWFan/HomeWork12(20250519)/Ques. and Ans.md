...
INCOME 係數的 95% 信賴區間為 [0.01283 , 0.04032]。
...
\[
\text{LIQUOR}_{it}= \beta_{1}+ \beta_{2}\,\text{INCOME}_{it}+u_{i}+e_{it},
\qquad 
u_{i}\sim\bigl(0,\sigma_{u}^{2}\bigr),\;
e_{it}\sim\bigl(0,\sigma_{e}^{2}\bigr)
\]

* 估計值  
  \[
  \hat{\beta}_{2}=0.02658,\qquad \text{SE}=0.00701.
  \]
...
LM 檢定統計量為 4.5475。若無未觀察個體異質性，LM 服從標準常態分配，其 5% 臨界值為 ±1.96。由於 4.5475 超過臨界值，拒絕虛無假設 
...
* 計得  
  \[
  \text{LM}=4.5475 
  \quad(\text{軟體亦報 } \text{LM}^{2}=20.68\sim\chi^{2}_{(1)}).
  \]

* 5 % 臨界值：標準常態 ±1.96  
  或  
  \(\chi^{2}_{0.95,1}=3.841\).

* 結論  
  \[
  \boxed{\text{拒絕 }H_{0}\!:\sigma^{2}_{u}=0}
  \]
  ⇒ 存在顯著的戶別異質性，應使用 FE/RE，而非 pooled OLS。
...
在模型中加入戶別平均所得 INCOMEM，得到LIQUOR^𝑖𝑡=0.91633+0.02074INCOME𝑖𝑡+0.00658INCOMEM𝑖
​
                                             (標準誤)(0.02220)
...
* 新增戶別平均收入  
  ...
  \[
  \text{INCOMEM}_{i}= \frac{1}{T}\sum_{t=1}^{T}\text{INCOME}_{it},
  \]

  並估計  

  \[
  \text{LIQUOR}_{it}= \beta_{1}+ \beta_{2}\,\text{INCOME}_{it}
                     +\gamma\,\text{INCOMEM}_{i}+c_{i}+e_{it}.
  \]

* 估計結果  

  \[
  \hat{\gamma}=0.00658,\qquad \text{SE}=0.02220,\qquad t=0.30
  \longrightarrow \text{不顯著}.
  \]

* 推論  
  戶別隨機效果 \(u_{i}\) 與收入 **無顯著相關** ⇒  
  RE 假設成立，(b) 的 RE 估計可安心使用。  
  （Hausman 對比檢定統計量 0.09 < 3.841，結論一致）
