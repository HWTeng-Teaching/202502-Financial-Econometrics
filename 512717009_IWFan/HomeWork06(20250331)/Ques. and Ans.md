...
> 對立假設：\( H_1: \beta_1 + 2\beta_2 \ne 5 \)

這屬於 **線性組合參數的檢定（General Linear Hypothesis）**，使用的統計量為 t 統計量。
...
### 🔢 Step 3：計算 \( \text{Var}(\hat{g}) \)（使用 Delta Method）
...
\[
t = \frac{\hat{g} - 5}{\text{se}(\hat{g})} = \frac{8 - 5}{3.317} \approx \frac{3}{3.317} \approx 0.905
\]
...
- 自由度 \( df = 63 - 3 = 60 \)
- 雙尾檢定，查表得 \( t_{0.025, 60} \approx 2.000 \)
...
- \( |t| = 0.905 < 2.000 \)，落在接受域  
- ❌ 無法拒絕虛無假設 \( H_0: \beta_1 + 2\beta_2 = 5 \)  
- 換句話說：在 5% 顯著水準下，**沒有足夠證據**顯示此線性組合不等於 5。
