...
**結論：**  
因為 統計量GQ 沒有在落在拒絕區內(0.8377＜1.173＜1.1968) ⇒ **無法拒絕 \( H_0 \)**  
=> 無足夠證據表明兩性之間的誤差變異數不同。
...
$$
σ²_ SINGLE = \frac{56231.0382}{400-5} = 142.357 
$$
...
由於 F 統計量 = 1.0538  沒有在落在拒絕區內(0.9453 ＜1.0538 ＜1.0581)落在不拒絕區間 ⇒ **無法拒絕 \( H_0 \)** 

表示在 5% 顯著水準下，**沒有證據認為男性與女性的誤差變異不同**。
...
1. 使用 OLS 殘差平方 ê²
2. 對 log(ê²) 進行回歸，設定異質變異來源為 metro 與 exper     
3. 擬合後取得預測值 ĥᵢ , 進行加權最小平方法（WLS）


FGLS 模型估計結果：

| 變數             | Estimate  |  SE      | 95% 信賴區間              |
|------------------|-----------|----------|---------------------------|
...
- **FGLS 或 OLS robust SE**：如果模型存在顯著的異質變異數（如(b)(c)題已顯示存在明顯異質變異數（NR² 與 White Test 結果顯著）），建議使用   
- **FGLS 有效提升估計效率**：可同時修正誤差異質性，但其估計需要對變異結構做出假設。
- **FGLS + Robust SE** ：可進一步強化模型推論的穩健性，標準誤與信賴區間穩定，顯示異質變異已成功控制

**FGLS + Robust SE**兼顧效率與穩健性最平衡的選擇，特別適合處理如 wage 模型這類具明顯誤差異質的經濟資料。
